...
- Shared navigation: `components/navigation.js`
...
Shared navigation is generated by `components/navigation.js`, loaded with `defer` on all pages. Keep other page content static; no framework or build step is required.
...
<header class="site-header">
  <a class="brand" href="../index.html">TAKUMI Lab</a>
  <nav aria-label="Primary navigation" data-navigation="primary">
    <a href="../index.html">Home</a>
  </nav>
</header>
```

Top page uses `href="index.html"` for the brand. App subpages use `href="../index.html"`. Internal overview links explicitly include `index.html` (for example, `swifkit/index.html`) so they work both on GitHub Pages and when HTML files are opened directly.
...
- navigation gap: `12px 24px`, wrapping as needed
- header gap: `16px 24px`; vertical padding: `16px`
- mobile header stacks brand above navigation; navigation gap: `10px 18px`
- global navigation includes Home, About, Projects, Team, Contact, and Apps, defined once in `navigationItems` in `components/navigation.js`
- Apps links to `index.html#apps`; paths resolve relative to the shared script so nested pages and site subpaths work
- load `components/navigation.js` on root pages and `../components/navigation.js` on app pages using a classic script with `defer`
- the HTML Home link is a fallback if JavaScript is disabled or unavailable
- app-specific links are retained in a separate footer navigation labeled `App navigation`
- current global page uses `aria-current="page"` and `--text`; Home at `#apps` highlights Apps, including on hash changes and history restoration; app subpages have no exact global-page match
...
Keep the footer simple. The footer also links to About, Projects, Team, and Contact in a wrapping navigation row with a 12px top margin. Use `data-navigation="footer"`; the shared script selects entries with `footer: true` from the same `navigationItems` list. An HTML Home link remains as a no-JavaScript fallback. Social links remain TODO comments until verified URLs are supplied.
...
1. Shared header
2. Shared global navigation; app-specific links remain in the footer
